...
;Напишете функция, която проверява дали в дадено дърво има слой (ниво) с определена сума на елементите.
  (define (sum-help t l) (cond ((null? t) 0)
        ((= 0 l) (root-tree t))
        (else (+ (sum-level? (left-tree t) s (- l 1))
              (sum-level? (right-tree t) s (- l 1))))))
...
(sum-level? sample-bst 0 1)

;Напишете функция, която по подадено дърво и две стойности на елементи намира
;най-ниско разположения елемент, сред чиито наследници се срещат тези две стойности.
